...
public final class TimeRewindAbility implements ConditionalAbility {
    ...
    public boolean tryActivate(ServerPlayerEntity player) {
        ...
        Optional<TemporalSnapshot> stored = PlayerStateHistory.get(player, REWIND_TICKS);
        if (stored.isEmpty()) {
            player.sendMessage(Text.literal("Not enough history to rewind"), true);
            return false;
        }

        TemporalSnapshot snapshot = stored.get();
        if (!snapshot.dimensionId().equals(TemporalSnapshotAdapter.dimensionId(player.getWorld()))) {
            player.sendMessage(Text.literal("Cannot rewind across dimensions"), true);
            return false;
        }

        ServerWorld world = player.getServerWorld();
        Optional<Vec3d> safePosition = SafeRewindPositionFinder.find(
                world,
                player,
                TemporalPositionAdapter.toMinecraft(snapshot.position())
        );
        ...
        player.setVelocity(TemporalPositionAdapter.toMinecraft(snapshot.velocity()));
